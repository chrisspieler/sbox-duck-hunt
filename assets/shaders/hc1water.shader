...
PS
{
    ...
    float4 MainPs( PixelInput i ) : SV_Target0
    {
		...
		//
		float3 vClear = Tex2DLevel( g_tFrameBufferCopyTexture, vScreenUv * g_vFrameBufferCopyInvSizeAndUvScale.zw, 0 ).xyz;
		// vClear = SrgbGammaToLinear( vClear );
		float3 vClearRefracted = Tex2DLevel( g_tFrameBufferCopyTexture, vRefractionUv * g_vFrameBufferCopyInvSizeAndUvScale.zw, 0 ).xyz;
		// vClearRefracted = SrgbGammaToLinear( vClearRefracted );
		// SrgbGammaToLinear makes everything way too dark. Why?
		...
		//
		{
			// m.Albedo *= lerp( g_vColor, g_vDeepColor, saturate( 1 / invCameraToDepth ) );
